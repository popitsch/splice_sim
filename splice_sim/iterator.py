from splice_sim.genomic_iterators import BAM_FUNMAP, BAM_FSECONDARY, BAM_FQCFAIL, BAM_FDUP, BAM_SUPPLEMENTARY
...
class SimulatedRead:
    ...
    def __repr__(self):
        return "\t".join(
            [self.name, self.chromosome, str(self.start), str(self.end), str(self.splicing), str(self.spliceSites)])
...
class SimulatedReadIterator:
    def __init__(self, readIterator,
                 flagFilter=BAM_FUNMAP | BAM_FSECONDARY | BAM_FQCFAIL | BAM_FDUP | BAM_SUPPLEMENTARY):
    ...
    def __next__(self):
        ...
        true_tid, true_strand, true_isoform, read_tag, true_chr, true_start, true_cigar, n_seqerr, n_converted, is_converted_read = name.split(
            '_')
        ...
        simulatedRead = SimulatedRead(name, chromosome, start + 1, end, spliced, spliceSites, read, true_tid,
                                      trueSpliced)
