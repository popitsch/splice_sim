"""
    ...
    Simulation methods for splice_sim
"""
import random
import sys
...
def postfilter_bam(config, bam_in, out_dir):
    ...
    for read in samin.fetch(
            until_eof=True):  # NOTE important to add until_eof=True otherwise unmapped reads will be skipped.
        if read.is_secondary or read.is_supplementary:
            f_reads += 1
            continue
        # map isoform to color
        true_tid, true_strand, true_isoform, read_tag, true_chr, true_start, true_cigar, n_seqerr, n_converted, is_converted_read = read.query_name.split(
            '_')
        ...
        for r in sam.fetch(until_eof=True):
            ...
            if not transcript_strand == read_strand:
                ...
                for op, l in r_cigartuples:
                    ...
                    while diff > 0:
                        if op == 8 and l > 1:
                            ...
                            print(
                                'ignoring seq diff stretch for ' + r.query_name + ', ' + r.cigarstring)  # TODO we need to insert a splice cigartuple
                        matchlen = l - diff
                        if matchlen > 0:
                        ...
                        if nblock is None:
                            print("ERR: no more ablocks in iso " + str(
                                iso) + ", read " + r.query_name + ', ' + r.cigarstring)  # should not happen :-/
                ...
                read.query_name = '_'.join([str(x) for x in [r.query_name,  # true_tid, true_strand, true_isoform, tag
                                                             ...
                                                             read.cigarstring,
                                                             # cigarstring for calculating aligned blocks
                                                             len(mismatches) + n_insertions + n_deletions,
                                                             # number of seq errors
                                                             0,  # number of conversions
                                                             is_converted_read
                                                             # read is chosen from the binomial distribution
                                                             ]])
...
def modify_bases(chrom, start, ref, alt, seq, is_reverse, conversion_rate, ref_pos=None, snp_pos=None, snps=None):
    ...
    for i, c in enumerate(seq):
        ...
        if (snp_pos is not None) and (ref_pos is not None) and (ref_pos[i] in snp_pos):
            ...
            if (not snp_strand) or (snp_strand == 'NA') or (is_reverse and (snp_strand == '-')) or (
                    (not is_reverse) and (snp_strand == '+')):
...
def add_read_modifications(m, in_bam, out_bam, conversion_rate, threads, tag_tc="xc", tag_isconvread="xm",
                           overwrite=False):
    ...
    with pysam.AlignmentFile(out_bam + '.tmp.bam', "wb", header=sam.header) as bam_out:
        for r in sam.fetch(until_eof=True):
            ...
            if is_converted_read == 1:  # read chosen from binomial; modify its bases
                ref_pos, snp_pos, snps = (
                [x + 1 if x is not None else x for x in r.get_reference_positions(full_length=True)],
                m.snp_pos_per_chr[true_chr] if true_chr in m.snp_pos_per_chr else None, m.snps) if (
                            m.snps is not None) else (None, None, None)  # note ref_pos are 0-based

                # print(ref_pos, snp_pos, snps, m.snp_pos_per_chr, m.snps)
                r.query_sequence, n_convertible, n_modified = modify_bases(true_chr, true_start, ref, alt,
                                                                           r.query_sequence,
                                                                           r.is_reverse, conversion_rate, ref_pos,
                                                                           snp_pos, snps)
            else:
            ...
            r.query_name = '_'.join([str(x) for x in
                                     [true_tid, true_strand, true_isoform, read_tag, true_chr, true_start, true_cigar,
                                      n_seqerr, n_converted, is_converted_read]])
