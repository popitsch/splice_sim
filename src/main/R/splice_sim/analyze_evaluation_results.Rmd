...
params:
    ...
    # slam_out_dir:
    #    value: "/Volumes/groups/ameres/Niko/projects/Ameres/splicing/splice_sim/testruns/big3_biseq_nf/slam_out/"
    # out_dir:
    ...
    # slam_out_dir:
    #     value: "/Volumes/groups/ameres/Niko/projects/Ameres/splicing/splice_sim/testruns/big3_slamseq_nf/slam_out/"
    # out_dir:
    ...
    slam_out_dir:
        value: "/Volumes/groups/ameres/Niko/projects/Ameres/splicing/splice_sim/testruns/big4_slamseq_nf/slam_out/"
    out_dir:
...
if (file.exists(data_file) ) {
  ...
  # load truth data
  isoform_truth=load_table(paste0(home_dir, '/sim/reference_model/isoform_truth.tsv.gz'))
  ...
  for ( m in c( 'truth', names(conf$mappers))) {
    for ( cr in conversion_rates) {
      ...
      pre=paste0(home_dir, '/eva/splice_sites_performance/', conf$dataset_name, '.cr',cr, '.', m)
...
tab = d[['tid_performance']] %>% 
  filter(rnk.x==1, condition_id=='0.1') %>% 
  ...
  merge(gene_anno, by.x = 'tid', by.y='transcript_id') %>% 
  merge(extra_transcript_table, by.x = 'tid', by.y='transcript_id')
...
tab = d[['tid_performance']] %>% 
  mutate(num_exons=case_when(rnk.x<=5 ~ as.character(rnk.x), 
                             rnk.x>5 ~ '>5',
                             T ~ 'NA'),
...
plot_corr = function(d, a, b, subtitle='', xlog=F, draw_diag=T) {
  thecor = paste("r_pearson = ", round(cor(d[[a]], d[[b]], use = "complete.obs"), 4), 
                 ...
                 "\nn =",nrow(d)  )
  p = ggplot( d %>% filter(!is.na(mapper)), aes_string(x=a, y=b, col='iso') ) +
...
plot_gene = function(gname) {
  tid=gn2tid(gname)
  if (params$plot_type=='by_condition') {
    tid_performance%>% filter(tid==!!gn2tid(gname)) %>% ggplot(aes(x=condition_id, y=F1, col=mapper, linetype=iso, group=paste0(mapper, iso))) + 
      geom_line() + geom_point() +
      ggtitle( paste0("F1 for ", gname) )
  } else {
  tid_performance%>% filter(tid==!!gn2tid(gname)) %>% ggplot(aes(x=mapper, y=F1, col=mapper, shape=iso, group=paste0(mapper, iso ))) + 
    geom_line() + geom_point() +
    ggtitle( paste0("F1 for ", gname) ) + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  }
...
tab = d[['splice_site_mappability']] %>% mutate(
  ...
  mutate(don_win_map_cat=case_when(
            mean_don_win_min_map<0.2 ~ 'low',
            mean_don_win_min_map>0.9 ~ 'high',
            T ~ 'medium'),
         acc_win_map_cat=case_when(
            mean_acc_win_min_map<0.2 ~ 'low',
            mean_acc_win_min_map>0.9 ~ 'high',
            T ~ 'medium')
    ) %>% 
