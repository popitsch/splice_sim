...
counts_per_feature = d[['feature_counts_mismapped']] %>% 
  left_join(d[['feature_counts']], by=c('tid', 'chromosome', 'start', 'end', 'ftype', 'mapper', 'condition_id' ) )

counts_per_tx = counts_per_feature%>% 
  group_by(mapper, condition_id, tid, ftype) %>% 
  summarise(FP=sum(FP), FN=sum(FN), read_count=sum(read_count), length=sum(length)) %>% 
  left_join(d[['gene_anno']] %>% select(tid, gene_name, gene_type, chromosome,start,end,strand,rnk,transcript_len), by='tid') %>% 
...
ggsave(paste0(params$out_dir,'/fpfn_per_feature.pdf'), width = 12, height = 8)
...
# ==============================================
tab = best_mapper %>% 
  select(tid, filter_type, best_mapper, best_diff, diff_to_other) %>% 
  arrange(chromosome, start)
