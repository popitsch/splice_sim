...
intron_mappability = d[['splice_site_performance']] %>% 
    ...
    group_by(mapper, tid, intron_id) %>% summarise(
      filter_tc=sum(low_map)>sum(!low_map), # filter because in most conditions it is filtered
      filter_notc=low_map[condition==0]     # filter because in cond0 it is filtered
      ) %>% 
  ungroup()
...
intron_mappability %>% 
  pivot_longer(-c(mapper, tid, intron_id)) %>% 
  pivot_wider(names_from=mapper, values_from = value, id_cols = c('tid', 'intron_id', 'name')) %>% 
...
sfrac = d[['splice_site_performance']] %>% 
  ...
  summarise(true_mat=sum(spl_FN)+sum(spl_TP),
            ...
            # --- non-tc filtered ----
            true_mat_fil_notc=sum(spl_FN[!filter_notc])+sum(spl_TP[!filter_notc]),
            found_mat_fil_notc=sum(spl_FP[!filter_notc])+sum(spl_TP[!filter_notc]),
            true_pre_fil_notc=sum(don_FN[!filter_notc])+sum(don_TP[!filter_notc])+sum(acc_FN[!filter_notc])+sum(acc_TP[!filter_notc]),
            found_pre_fil_notc=sum(don_FP[!filter_notc])+sum(don_TP[!filter_notc])+sum(acc_FP[!filter_notc])+sum(acc_TP[!filter_notc]),
            n_fil_notc=sum(filter_notc, na.rm = T),
            # --- tc filtered ----
            true_mat_fil_tc=sum(spl_FN[!filter_tc])+sum(spl_TP[!filter_tc]),
            found_mat_fil_tc=sum(spl_FP[!filter_tc])+sum(spl_TP[!filter_tc]),
            true_pre_fil_tc=sum(don_FN[!filter_tc])+sum(don_TP[!filter_tc])+sum(acc_FN[!filter_tc])+sum(acc_TP[!filter_tc]),
            found_pre_fil_tc=sum(don_FP[!filter_tc])+sum(don_TP[!filter_tc])+sum(acc_FP[!filter_tc])+sum(acc_TP[!filter_tc]),
            n_fil_tc=sum(filter_tc, na.rm = T),
            .groups = 'drop'
         ) %>% 
  filter( (found_mat_fil_tc+found_pre_fil_tc>!!min_remaining_readcount) | 
          (found_mat_fil_notc+found_pre_fil_notc>!!min_remaining_readcount) ) %>% # drop if due to filtering the overall readcount became too low
  mutate(
         true_spliced=ifelse(true_pre>0,true_mat/(true_pre+true_mat), NA),
         found_spliced=ifelse(found_pre>0,found_mat/(found_pre+found_mat), NA),
         found_spliced_fil_notc=ifelse(found_pre_fil_notc>0,found_mat_fil_notc/(found_pre_fil_notc+found_mat_fil_notc), NA),
         found_spliced_fil_tc=ifelse(found_pre_fil_tc>0,found_mat_fil_tc/(found_pre_fil_tc+found_mat_fil_tc), NA)
  ) %>% 
  mutate(diff_spliced=true_spliced-found_spliced,
         diff_spliced_fil_notc=true_spliced-found_spliced_fil_notc,
         diff_spliced_fil_tc=true_spliced-found_spliced_fil_tc) %>% 
  mutate(improvement_notc=abs(diff_spliced) - abs(diff_spliced_fil_notc),
         improvement_tc=abs(diff_spliced) - abs(diff_spliced_fil_tc)) %>% 
  mutate(num_exons=case_when(rnk<=5 ~ as.character(rnk), 
                             rnk>5 ~ '>5',
                             T ~ 'NA'),
         mappability=case_when(mappability_mean<0.2 ~ 'low',
  ...
  select(mapper, tid, condition, n_fil_notc, n_fil_tc, 
         true_spliced, found_spliced, found_spliced_fil_notc, found_spliced_fil_tc, 
         true_mat, found_mat, found_mat_fil_notc, found_mat_fil_tc,
         true_pre, found_pre, found_pre_fil_notc, found_pre_fil_tc,
         everything())

# sanity ccheck: 
testthat::expect_that(sfrac %>% filter(n_fil_notc==0, diff_spliced<diff_spliced_fil_notc) %>% nrow(), equals(0))
testthat::expect_that(sfrac %>% filter(n_fil_tc==0, diff_spliced<diff_spliced_fil_tc) %>% nrow(), equals(0))

# number of tids 
sfrac_ntx = sfrac %>% summarise(n_tids=n_distinct(tid)) %>% pull(n_tids)
sfrac_ntx2 = sfrac %>% filter(n_fil_notc>0|n_fil_tc>0) %>% summarise(n_tids=n_distinct(tid)) %>% pull(n_tids)
...
sfrac %>% 
  select(mapper, tid, condition, n_fil_tc, n_fil_notc, 
         improvement_tc, improvement_notc, 
         mappability, n_overlapping, gene_type, rnk) %>% 
  filter(n_fil_tc + n_fil_notc > 0) %>% 
  mutate(is_overlapping=ifelse(n_overlapping>0,'overlapping tx', 'non-overlapping tx')) %>% 
  pivot_longer(-c(mapper, tid, condition, n_fil_tc, n_fil_notc, n_overlapping, mappability, is_overlapping, gene_type, rnk )) %>% 
...
```{r sj_corr_best_tx}
calc_best_mapper = function(tab, with_tc_conv) {
  if ( with_tc_conv ) {
    print("With TC")
    tab = tab %>% 
      select(mapper, tid, condition, n_filtered_introns=n_fil_tc, n_overlapping, true_spliced, found_spliced, found_spliced_fil_tc, mappability) %>% 
      group_by(mapper, tid, n_filtered_introns, n_overlapping, true_spliced,  mappability) %>% 
      summarise( m_found_spliced=median(found_spliced, na.rm=T), 
                 m_found_spliced_fil=median(found_spliced_fil_tc, na.rm=T))
  } else {
    print("W/O TC")
    tab = tab %>% 
      select(mapper, tid, condition, n_filtered_introns=n_fil_notc, n_overlapping, true_spliced, found_spliced, found_spliced_fil_notc, mappability) %>% 
      group_by(mapper, tid, n_filtered_introns, n_overlapping, true_spliced,  mappability) %>% 
      summarise( m_found_spliced=median(found_spliced, na.rm=T), 
                 m_found_spliced_fil=median(found_spliced_fil_notc, na.rm=T))
  }
  tab %>% 
  ungroup() %>% 
  pivot_wider(names_from=mapper, 
              values_from=c(m_found_spliced, m_found_spliced_fil, n_filtered_introns), 
              id_cols=c('tid', 'n_overlapping', 'true_spliced', 'mappability')) %>% 
...
p1 = calc_best_mapper(sfrac, with_tc_conv=F) %>% 
  group_by(mappability, after_fil) %>% 
  count(best_mapper) %>% 
  ggplot(aes(x=best_mapper, y=n, fill=after_fil)) +
  geom_bar(stat='identity') +
  facet_wrap(mappability~.) +
  ggtitle('No TC conversions' )

p2 = calc_best_mapper(sfrac, with_tc_conv=T) %>% 
  group_by(mappability, after_fil) %>% 
  count(best_mapper) %>% 
  ggplot(aes(x=best_mapper, y=n, fill=after_fil)) +
  geom_bar(stat='identity') +
  facet_wrap(mappability~.) +
  ggtitle('With TC conversions' )

my_plot_grid(list(p1,p2), 'Best mapper per tx wrt. fmat', ncol=1)
...
for ( m in c('STAR', 'HISAT3N')) {
  write_bed(
    ...
    mutate(score=ifelse(is.na(all_F1),0,all_F1*1000), 
           rgb=ifelse(filter_tc,'255,0,0','0,255,0'),
           start1=start,
           end1=end) %>% 
    select(chromosome,start,end,intron_id,score,strand,start1,end1,rgb),
    paste0(params$out_dir,'/filtered_introns_withTC.',m,'.bed'),
    paste0(m,"_filtered_introns_withTC")
  )
}
# W/o tc
for ( m in c('STAR', 'HISAT3N')) {
  write_bed(
    tab %>% filter(mapper==!!m) %>% 
    mutate(score=ifelse(is.na(all_F1),0,all_F1*1000), 
           rgb=ifelse(filter_notc,'255,0,0','0,255,0'),
           start1=start,
           end1=end) %>% 
    select(chromosome,start,end,intron_id,score,strand,start1,end1,rgb),
    paste0(params$out_dir,'/filtered_introns_noTC.',m,'.bed'),
    paste0(m,"_filtered_introns_noTC")
  )
}
...
for ( withtc in c(T,F)) {
 tab = calc_best_mapper(sfrac, with_tc_conv=withtc) %>% 
  select(tid, best_mapper, best_diff) %>% 
  left_join(d[['gene_anno']], by='tid') %>% 
  arrange(chromosome, start)
  write_bed(
    tab %>% 
    mutate(score=ifelse(is.na(best_diff),0,(1-best_diff)*1000), 
           rgb=case_when(
    ...
    paste0(params$out_dir,'/best_mapper_fmat',ifelse(withtc,'_withTC','_noTC'), '.bed'),
    paste0('best_mapper_fmat',ifelse(withtc,'_withTC','_noTC'), '.bed')
  )
...
tid=(sfrac %>% filter(diff_spliced>0, diff_spliced_fil_tc==0) %>% pull(tid)) [1]
intron_mappability %>% filter(tid==!!tid)

# for which tx do we have the highest correction
sfrac %>% arrange(desc(improvement_tc)) %>% select(improvement_tc, everything()) %>% head()

summarise_results = function(gene_name) {
  tid=(sfrac %>% filter(gene_name==!!gene_name) %>% pull(tid))[1]
  sfrac %>% filter(tid==!!tid) %>% select(improvement_tc, everything()) %>% print()
  intron_mappability %>% filter(tid==!!tid, filter_tc==T) %>% print()
}
