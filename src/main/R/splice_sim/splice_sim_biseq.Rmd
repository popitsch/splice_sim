...
params:
    splice_sim_config:
       value: "/groups/zuber/zubarchive/USERS/tobias/myProjects/slamdunk/splice_sim/big4_biseq_nf/splice_sim.config.json"
    out_dir:
       value: "/groups/zuber/zubarchive/USERS/tobias/myProjects/slamdunk/splice_sim/big4_biseq_nf/analysis/"
---
<style type="text/css">
body, td {
   font-size: 10px;
...
high = m[["tx"]] %>% dplyr::group_by(mappability) %>% dplyr::summarise(tx = n())  %>% dplyr::filter(mappability == "high") %>% .$tx
medium = m[["tx"]] %>% dplyr::group_by(mappability) %>% dplyr::summarise(tx = n())  %>% dplyr::filter(mappability == "medium") %>% .$tx
...
high = m[["fx"]] %>% dplyr::group_by(mappability) %>% dplyr::summarise(tx = n())  %>% dplyr::filter(mappability == "high") %>% .$tx
medium = m[["fx"]] %>% dplyr::group_by(mappability) %>% dplyr::summarise(tx = n())  %>% dplyr::filter(mappability == "medium") %>% .$tx
...
plotTab %>%
  ...
  ggtitle("Umap vs Bismap [transcript]")
...
plotTab %>%
  ...
  geom_point(size=0.5,alpha=0.8) +
  scale_color_identity() +
  ggtitle("Umap vs Bismap [features]")
...
plotTab %>%
  ...
  geom_point(size=0.5,alpha=0.8) +
  scale_color_identity() +
  ggtitle("Umap vs Bismap [SJ donor]")
...
plotTab %>%
  ...
  geom_point(size=0.5,alpha=0.8) +
  scale_color_identity() +
  ggtitle("Umap vs Bismap [SJ acceptor]")

plotTab = rbind(
  mbs[["tx"]] %>% dplyr::mutate(class = "bsmap") %>% dplyr::select(class, mappability, mean_map),
  m[["tx"]] %>% dplyr::mutate(class = "umap") %>% dplyr::select(class, mappability, mean_map)
)
