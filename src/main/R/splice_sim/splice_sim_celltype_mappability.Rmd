...
p1 <- top_genes %>% group_by(cat, ds, mappability) %>% count() %>% 
...
ribozero_genes = top_genes %>%
  dplyr::filter(cat == "RiboZero", mappability %in% c("medium","low")) %>%
...
#all_gene_sets = msigdbr(species = "Mus musculus", category = "C2")

ribozero_gene_sets = all_gene_sets %>%
  dplyr::filter(ensembl_gene %in% ribozero_genes)

plotTab = all_gene_sets %>%
  dplyr::group_by(gs_cat, gs_subcat) %>%
  dplyr::summarise(total = length(unique(gs_name))) %>%
  dplyr::mutate(name = paste0(gs_cat," ", gs_subcat)) %>%
  dplyr::left_join(
    ribozero_gene_sets %>%
      dplyr::group_by(gs_cat, gs_subcat) %>%
      dplyr::summarise(ribozero = length(unique(gs_name))) %>%
      dplyr::mutate(name = paste0(gs_cat," ", gs_subcat))
  )

pal = brewer.pal(3,"Set1")[c(2,1)]

p2 <- plotTab %>%
  ungroup %>%
  dplyr::mutate(name = factor(name, levels = c("H ",setdiff(plotTab$name,"H ")))) %>%
  dplyr::arrange(name) %>%
  dplyr::mutate(percentage_ribozero = ribozero / total * 100, percentage_rest = (total - ribozero) / total * 100) %>%
  dplyr::select(name, percentage_rest, percentage_ribozero) %>%
  gather(class, values, 2:3) %>%
  dplyr::mutate(class = case_when(
    class == "percentage_rest" ~ "other genes only",
    class == "percentage_ribozero" ~ "ribozero-containing",
  )) %>%
  dplyr::mutate(class = factor(class, levels = c("other genes only","ribozero-containing"))) %>%
  ggplot(aes(x = name, y = values, fill = class)) +
  ...
  xlab("") + ylab("Percentage of gene sets covered [%]") +
  ggtitle("MSigDb gene set coverage")

my_plot_grid(list(p1,p2), 'Ribo-zero gene statistics', labels = T)
ggsave(paste0(params$out_dir,'/qc_map_cat_dist.pdf'), width = 15, height = 6)
#+scale_y_log10()
...
plotTab %>%
  ...
  dplyr::mutate(name = factor(name, levels = c("H ",setdiff(plotTab$name,"H ")))) %>%
  dplyr::arrange(name) %>%
  dplyr::mutate(total = total - ribozero) %>%
  dplyr::select(name, total, ribozero) %>%
  gather(class, values, 2:3) %>%
  dplyr::mutate(class = factor(class, levels = c("total","ribozero"))) %>%
  ggplot(aes(x = name, y = values, fill = class)) +
  geom_bar(stat="identity") + 
  scale_fill_manual(values = pal) +
  theme(
...
plotTab %>%
  ...
  dplyr::select(name, percentage_rest, percentage_ribozero) %>%
  gather(class, values, 2:3) %>%
  dplyr::mutate(class = factor(class, levels = c("percentage_rest","percentage_ribozero"))) %>%
  ggplot(aes(x = name, y = values, fill = class)) +
  geom_bar(stat="identity") + 
  scale_fill_manual(values = pal) +
  theme(
...
plotTab = all_gene_sets %>%
  dplyr::group_by(gs_cat, gs_subcat) %>%
  dplyr::summarise(total = length(unique(gene_symbol))) %>%
  dplyr::mutate(name = paste0(gs_cat," ", gs_subcat)) %>%
  dplyr::left_join(
    ribozero_gene_sets %>%
      dplyr::group_by(gs_cat, gs_subcat) %>%
      dplyr::summarise(ribozero = length(unique(gene_symbol))) %>%
      dplyr::mutate(name = paste0(gs_cat," ", gs_subcat))
  )
...
plotTab %>%
  ...
  xlab("") + ylab("Percentage of genes covered [%]")
