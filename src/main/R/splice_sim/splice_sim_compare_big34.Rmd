...
big4_home_dir=paste0(dirname(params$big4_config),'/')
big4_conf=fromJSON(paste(readLines(params$big4_config), collapse=""))
...
# results files (~45 G) 
big3_data_file=paste0(big3_home_dir,'/eva/results/data.rds')
big4_data_file=paste0(big4_home_dir,'/eva/results/data.rds')
big3_meta_file=paste0(big3_home_dir,'/eva/results/meta.rds')
...
# shortcuts
tx3 = d3[['tx']] %>% left_join(m[['tx']], by=c('fid'='tid')) 
all_tids=unique(tx3 %>% pull(fid))
tx4 = d4[['tx']] %>% filter(fid %in% all_tids) %>% left_join(m[['tx']], by=c('fid'='tid')) 
...
p1 = perf_tx34 %>% 
  ...
  geom_boxplot() +
  facet_grid(mapper~conversion_rate)
