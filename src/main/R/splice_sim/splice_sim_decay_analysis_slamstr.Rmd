...
params:
    decay_config_tx:
       value: "/Volumes/groups/ameres/Niko/projects/Ameres/splicing/splice_sim/testruns/decay_sim_nf4_kss/config.json"
    decay_config_intron:
       value: "/Volumes/groups/ameres/Niko/projects/Ameres/splicing/splice_sim/testruns/decay_sim_nf4_intron_kss/config.json"
    decay_filter_experiment_folder:
       value: "/Volumes/groups/ameres/Niko/projects/Ameres/splicing/splice_sim/testruns/decay_sim_nf4_kss/filter_experiment"
    out_dir:
...
all_tids=decay_m[['tx']][['tp0']] %>% filter(len>100) %>% pull(tid)

# load slamstr counts
decay_d_slamstr=tibble()
for ( tp in decay_conf_tx$timepoints) {
  for (m in c('truth', 'STAR.final', 'HISAT3N.final')) {
    decay_d_slamstr=decay_d_slamstr %>% rbind(
      load_table(paste0(decay_home_dir_tx, '/slamstr/counts/', decay_conf_tx$dataset_name, '_tp',tp,'.cr0.05.',m,'.counts.tsv.gz')) %>% 
        ...
        mutate(mapper=case_when(
          ...
          T ~ 'NA')) %>% 
        mutate(ftc=tc_reads/all_reads)
    )
  }
}

# correlate simmulated  values with slamstr derived values from simulated alignment
comp = 
  decay_d_slamstr  %>% filter(mapper %in% c('simulated')) %>% select(fid=feature_id, tp, mapper, slamstr_ftc=ftc) %>% 
  left_join(
  decay_fcr_tx %>% filter(mapper %in% c('simulated')) %>% select(fid, tp, mapper, sim_ftc=ftc),
  by=c('fid', 'tp', 'mapper')
  ) %>% left_join(decay_m[['tx']][['tp0']], by=c('fid'='tid'))
comp %>% mutate(is_overlapping=overlapping_tids!='') %>% ggplot(aes(slamstr_ftc, sim_ftc, col=is_overlapping)) + geom_point() +
  facet_wrap(tp~.)
comp %>% filter(tp==20,overlapping_tids=='') %>% mutate(diff=abs(slamstr_ftc-sim_ftc)) %>% arrange(desc(diff)) %>% head(10)
```
