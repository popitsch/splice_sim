...
params:
    splice_sim_config:
       value: "/groups/zuber/zubarchive/USERS/tobias/myProjects/slamdunk/splice_sim/big5_slamseq_nf/splice_sim.config.json"
    out_dir:
       value: "/groups/zuber/zubarchive/USERS/tobias/myProjects/slamdunk/splice_sim/big5_slamseq_nf/analysis/"
---
<style type="text/css">
body, td {
   font-size: 10px;
...
intron_mappability = intron_mappability %>% 
  distinct %>%
  dplyr::group_by(tid, fid, filter_type,mapper) %>%
  dplyr::summarise(filtered = max(filtered)) %>%
  ungroup
...
intron_mappability %>% 
  ...
  dplyr::select(mapper, tid, fid, filter_type, mappability, filtered) %>%
  distinct %>%
...
cov_comp_tx = cache({
  comp_cov(tx, tx20, c('mapper', 'conversion_rate', 'fid', 'len', 'classification' ))
  }, 'cov_comp_tx', rerun=F)

cov_comp_fx = cache({comp_cov(fx, fx20, c('mapper', 'conversion_rate', 'fid', 'true_isoform', 'ftype', 'len', 'classification' ))
}, 'cov_comp_fx', rerun=F)
# sanity check: 
...
perf_comp = cache({tx %>% #bind_rows(fx) %>% 
...
upsetlist = list(mat = perf %>%
  dplyr::filter(mapper == "HISAT3N", mappability == "medium",ftype == "exon",true_isoform == "mat") %>% .$fid,
  pre = perf %>%
  dplyr::filter(mapper == "HISAT3N", mappability == "medium",ftype == "exon",true_isoform == "pre") %>% .$fid
)

perf %>%
  dplyr::group_by(mapper, mappability, ftype, true_isoform) %>%
  dplyr::summarise(NAs = sum(is.na(F1))) %>%
  ggplot(aes(x = ftype, y = NAs, fill = true_isoform)) +
  geom_bar(stat= "identity") +
  facet_grid(mappability~mapper, scales = 'free_y')
