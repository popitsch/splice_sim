...
params:
    ...
    splice_sim_config_r2:
       value: "/groups/ameres/Niko/projects/Ameres/splicing/splice_sim/testruns/big4_slamseq_nf_rep2/splice_sim.config.json"
    splice_sim_config_r3:
       value: "/groups/ameres/Niko/projects/Ameres/splicing/splice_sim/testruns/big4_slamseq_nf_rep3/splice_sim.config.json"
    decay_config:
...
home_dir_r2=paste0(dirname(params$splice_sim_config_r2),'/')
home_dir_r3=paste0(dirname(params$splice_sim_config_r3),'/')
...
# results files (~45 G) 
meta_file=paste0(home_dir_r1,'/results/meta.rds') # read from 1st replicate
data_file_r1=paste0(home_dir_r1,'/results/data.rds')
data_file_r2=paste0(home_dir_r2,'/results/data.rds')
data_file_r3=paste0(home_dir_r3,'/results/data.rds')
  
d=list()
d[['rep1']] = load_rds(data_file_r1)
d[['rep2']] = load_rds(data_file_r2)
d[['rep3']] = load_rds(data_file_r3)

m=load_rds(meta_file)

# shortcuts
tx = d[['rep1']][['tx']] %>% mutate(rep='rep1') %>% 
  bind_rows(
    d[['rep2']][['tx']] %>% mutate(rep='rep2') 
  ) %>% 
  bind_rows(
    d[['rep3']][['tx']] %>% mutate(rep='rep3') 
  ) %>% 
  left_join(m[['tx']], by=c('fid'='tid')) %>% 
  mutate(rep=factor(rep, levels=c('rep1', 'rep2', 'rep3')))

fx = d[['rep1']][['fx']] %>% mutate(rep='rep1') %>% 
  bind_rows(
    d[['rep2']][['fx']] %>% mutate(rep='rep2') 
  ) %>% 
  bind_rows(
    d[['rep3']][['fx']] %>% mutate(rep='rep3') 
  ) %>% 
...
# bam stats
# ===========================================================
if (dir.exists(paste0(home_dir_r1,'/eva/bamstats'))) {
  d[['bam_stats']]=tibble()
  for ( f in list.files(paste0(home_dir_r1,'/eva/bamstats'), pattern='\\.bamstats.tsv$', full.names = T)) {
    d[['bam_stats']] = d[['bam_stats']] %>% bind_rows(load_table(f) %>% mutate(rep='rep1'))
  }
  for ( f in list.files(paste0(home_dir_r2,'/eva/bamstats'), pattern='\\.bamstats.tsv$', full.names = T)) {
    d[['bam_stats']] = d[['bam_stats']] %>% bind_rows(load_table(f) %>% mutate(rep='rep2'))
  }
  for ( f in list.files(paste0(home_dir_r3,'/eva/bamstats'), pattern='\\.bamstats.tsv$', full.names = T)) {
    d[['bam_stats']] = d[['bam_stats']] %>% bind_rows(load_table(f) %>% mutate(rep='rep3'))
  }
  d[['bam_stats']]=d[['bam_stats']] %>% mutate(rep=factor(rep, levels=c('rep1', 'rep2', 'rep3')))
}
...
p1 = bam_stats %>% 
  ...
  geom_col() +
  ylab("# reads") + xlab("conversion rate [%]")
...
p2 = bam_stats %>%
  ...
  my_scales() + 
  ylab("") + xlab("conversion rate [%]")


my_plot_grid(list(p1,p2), 'bam stats', labels = T)
...
for ( r in c('rep1', 'rep2', 'rep3')) {
  plt[[r]] = 
    count_na_per_column(c(setNames(d[[r]], paste0('d_',names(d[[r]]))))) %>% 
...
for ( r in c('rep1', 'rep2', 'rep3')) {
  ...
  found_tids=unique(d[[r]][['tx']] %>% pull(fid))
  ...
  p1=missing_tx %>% ggplot(aes(x=factor(cat), y=len)) + 
    geom_boxplot()  +scale_y_log10() + ggtitle("Length distribution of recovered\nvs missing tx") +
    geom_hline(yintercept = conf$readlen, col='red')
  p2=missing_tx %>% ggplot(aes(x=factor(cat), y=ilen)) + 
    geom_violin()  +scale_y_log10() + ggtitle("Intron Length distribution of recovered\nvs missing tx") +
    geom_hline(yintercept = conf$max_ilen, col='red')
  # did we recover all reads?
  p3 = d[[r]][['tx']] %>% group_by(mapper, conversion_rate, classification) %>% summarise(count=sum(count)) %>% filter(classification %in% c('FN', 'TP', 'FP_raw')) %>% ggplot(aes(conversion_rate, count, fill=classification)) + geom_col() + facet_wrap(mapper~.) + ggtitle('read counts per mapper/condition') 
  plt[[r]] = my_plot_grid(list(p1,p2,p3), r, nrow = 1)
}
my_plot_grid(plt, 'QC: missing tx', ncol = 1)
ggsave(paste0(params$out_dir, 'qc_missing_tids.pdf'), width=12, height=10)

```
### Isoform selection
```{r}
# see /groups/ameres/Niko/projects/Ameres/splicing/data/200513_R9575_pulse_reseq/3_map/STAR/mm10/2_pickMaxIsoform.sh
isoform_mix = load_table('/groups/ameres/Niko/projects/Ameres/splicing/data/200513_R9575_pulse_reseq/3_map/STAR/mm10/ns_r1_ip_180/ns_r1_ip_180.gencode.vM21.annotation.nochr.sorted.MAXISOFORM.gff3.data.tsv')
p1=isoform_mix %>% count(mode) %>% ggplot(aes(mode, y=n)) + geom_col()
p2=isoform_mix %>% count(isCanonical) %>% ggplot(aes(isCanonical, y=n)) + geom_col()

glue("We mostly chose the canonical tx. frac= {isoform_mix %>% summarise(n=sum(isCanonical=='yes')) %>% pull(n) / isoform_mix %>% nrow()},")
my_plot_grid(list(p1,p2), labels=T, 'Isoform selection', nrow = 1)
```
...
tab = tx %>% # NB too-short tx are lost
  ...
  group_by(rep, fid, mapper, true_isoform, classification, mappability,is_overlapping ) %>% 
...
cov_tx = cache({
  calc_coverage(tx %>% group_by(rep, mapper, conversion_rate, fid, len, classification ) ) 
}, 'cov_tx', rerun = F)

cov_tx2 = cache({
  calc_coverage(tx %>% group_by(rep, mapper, conversion_rate, fid, true_isoform, len, classification ) ) 
}, 'cov_tx2', rerun = F)

cov_fx = cache({
 calc_coverage(fx %>% group_by(rep, mapper, conversion_rate, fid, true_isoform, ftype, len, classification ))
}, 'cov_fx', rerun = F)

cov_fx2 = cache({
  calc_coverage(fx %>% group_by(rep, mapper, conversion_rate, fid, ftype, len, classification ))
}, 'cov_fx2', rerun = F)



p1 = cov_tx2 %>% filter(rep=='rep1') %>% mutate(ftype='tx') %>% select(rep, mapper, ftype, true_isoform, value) %>% 
  bind_rows(cov_fx %>% filter(rep=='rep1') %>% select(rep, mapper, ftype, true_isoform, value)) %>% 
  ...
  ggtitle('Coverage per genomic feature, replicate 1') +
...
p2 = cov_fx %>% select(rep, mapper, conversion_rate, ftype, true_isoform, value) %>% 
  group_by(rep, conversion_rate, ftype, true_isoform, mapper) %>% 
  calc_ci(value) %>% 
  ggplot(aes(x=as.numeric(as.character(conversion_rate))*100, y=col.mean, col=mapper, linetype=rep, group=paste0(rep,mapper))) + 
  geom_line() +
  geom_ribbon(aes(ymin=col.lower, ymax=col.upper), linetype=0, alpha=0.2) +
  ggtitle('Mean coverage per condition/replicate') +
  facet_grid(rep~true_isoform+ftype, scales = 'free') +
  xlab("conversion rate [%]") + ylab("mean coverage") +
  my_scales()


my_plot_grid(list(p1,p2), labels=T, main='Coverage per mapper', nrow = 1, rel_widths = c(1, 1.5))
...
remove(cov_tx, cov_tx2, cov_fx, cov_fx2 )
...
perf_tx = cache({
  tx %>% 
    group_by(rep, fid, mapper, conversion_rate, classification, len, true_isoform, mappability, mean_map, GC, 
...
perf_fx = cache({
  # too large to calc in 1 run. split by rep, calc and concat
  split_fx=list()
  for (r in levels(fx$rep)) {
    print(paste0("calculating rep ", r ))
    split_fx[[r]] = fx %>% 
      filter(rep==!!r) %>% 
      select(-rnk) %>% 
      rename(rnk=tx_rnk) %>% #mappability=tx_mappability; we could also use tx_mappability here?
...
perf %>% group_by(rep, conversion_rate, mapper, mappability, true_isoform, ftype) %>% calc_iqr(precision) %>% 
  ggplot(aes(conversion_rate, col.median, col=mapper, group=paste0(mapper,true_isoform), linetype=true_isoform)) +
  geom_line() +
  #geom_ribbon(aes(ymin=col.lower, ymax=col.upper), linetype=2, alpha=0.2) +
  facet_grid(mappability~ftype+rep) +
  ylab("") +
  ggtitle('Median precision per condition') 

perf %>% group_by(rep, conversion_rate, mapper, mappability, true_isoform, ftype) %>% calc_iqr(recall) %>% 
  ggplot(aes(conversion_rate, col.median, col=mapper, group=paste0(mapper,true_isoform), linetype=true_isoform)) +
  geom_line() +
  #geom_ribbon(aes(ymin=col.lower, ymax=col.upper), linetype=2, alpha=0.2) +
  facet_grid(mappability~ftype+rep) +
...
perf %>% group_by(rep, conversion_rate, mapper, mappability, true_isoform, ftype) %>% calc_iqr(F1) %>% 
  ggplot(aes(conversion_rate, col.median, col=mapper, group=paste0(mapper,true_isoform), linetype=true_isoform)) +
  geom_line() +
  #geom_ribbon(aes(ymin=col.lower, ymax=col.upper), linetype=2, alpha=0.2) +
  facet_grid(mappability~ftype+rep, scales = 'free') +
...
perf_tx %>% filter(mappability=='high') %>% 
  group_by(rep, conversion_rate, mapper, true_isoform, num_exons) %>% calc_iqr(F1) %>% 
  ggplot(aes(conversion_rate, col.median, col=mapper, group=paste0(mapper,true_isoform), linetype=true_isoform)) +
  geom_line() +
  #geom_ribbon(aes(ymin=col.lower, ymax=col.upper), linetype=2, alpha=0.2) +
  facet_grid(num_exons~rep) +
  scale_y_continuous(sec.axis = sec_axis(~ . , name = "genomic mappability", breaks = NULL, labels = NULL)) +
  theme(axis.title.y.right = element_text(angle=-90, vjust = 1.5))+
  ylab("F1") +
...
perf %>% 
  ...
  facet_grid(true_isoform+mapper~ftype+rep) +
...
perf_fx %>% 
  filter(ftype=='exon') %>% 
  group_by(rep, conversion_rate, mapper, mappability, ftype, true_isoform, convertibility) %>% 
  calc_iqr(F1) %>% 
  ggplot(aes(conversion_rate, col.median, col=mapper, group=paste0(mapper,true_isoform), linetype=true_isoform)) +
  geom_line() +
  #geom_ribbon(aes(ymin=col.lower, ymax=col.upper), linetype=2, alpha=0.2) +
  facet_grid(mappability~convertibility+rep, scales = 'free', labeller = label_both) +
  ylab("") +
...
results[['map_fx_tab']] = fx %>% filter(ftype=='exon') %>% calc_map_table() %>% 
  mutate(best_mapper_no_conv = ifelse(F1_0_HISAT3N-F1_0_STAR>0.05, 'HISAT3N',
                               ifelse(F1_0_HISAT3N-F1_0_STAR< -0.05,'STAR','BOTH')),
         best_mapper_conv = ifelse(F1_HISAT3N-F1_STAR>0.05, 'HISAT3N',
                            ifelse(F1_HISAT3N-F1_STAR< -0.05,'STAR','BOTH'))) %>% 
...
fcr = cache({
  tx %>% 
    bind_rows(fx) %>% 
    group_by(rep, fid, mapper, conversion_rate, classification, true_isoform, mappability, ftype) %>% 
...
fcr_perf = cache({
  fcr %>%
    select(rep, fid, conversion_rate,true_isoform,ftype,mappability,true_fcr1, found_fcr1) %>% 
    ungroup() %>% 
...
fcr_perf %>%
  ...
  group_by(rep, conversion_rate,true_isoform,ftype, name) %>% 
  calc_ci(value) %>% 
  ...
  facet_grid(true_isoform~ftype+rep)
...
fcr_perf %>% group_by(rep, conversion_rate, ftype, true_isoform) %>% 
  count(best_mapper) %>% 
  mutate(frac = n / sum(n)) %>% 
...
fcr_tab %>% 
  ...
  ggplot(aes(best_mapper, n, fill=best_mapper)) +
    geom_bar(stat='identity') +
    facet_wrap(rep~ftype, scales = 'free') +
...
p1 = sj %>% filter(mapper=='HISAT3N') %>% 
  group_by(rep, true_isoform, classification, class_type) %>% summarise(count=sum(count)) %>% 
  ggplot(aes(classification, count, fill=true_isoform)) + geom_bar(stat='identity', position='dodge') + 
  facet_grid(class_type~true_isoform+rep) +
  ggtitle('HISAT3N') + theme(legend.position="none")
p2 = sj %>% filter(mapper=='STAR') %>% 
  group_by(rep, true_isoform, classification, class_type) %>% summarise(count=sum(count)) %>% 
  ggplot(aes(classification, count, fill=true_isoform)) + geom_bar(stat='identity', position='dodge') + 
  facet_grid(class_type~true_isoform+rep) +
...
sj_per = cache({
  sj %>% 
    group_by(rep, mapper, conversion_rate, fid, class_type, classification) %>% 
    summarise(count=sum(count)) %>% 
...
intron_mappability = cache({
  sj_per %>% 
    group_by(rep, tid, mapper, conversion_rate) %>% 
...
intron_mappability %>% 
  ...
  group_by(rep, filter_type, mappability, lab) %>% 
  count() %>% 
  group_by(rep, filter_type, mappability) %>% 
  mutate(s=sum(n), frac=n/s) %>% 
  ggplot(aes(x=lab, y=frac, fill=lab)) +
  scale_fill_manual(name = "lab",values = setNames(brewer.pal(length(levs),"Set1"), levs)) +
  geom_bar(stat='identity') +
  facet_grid(mappability~filter_type+rep, scales = 'free') +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  ggtitle('Filter results per intron') +
...
sfrac_conv = cache({
  sj_per %>% 
    left_join(intron_mappability, by=c('rep', 'mapper', 'fid', 'tid' )) %>% 
    filter(filter_type=='include_converted') %>%  # TC part
    group_by(rep, tid, mapper, conversion_rate, filter_type, tx_mappability) %>% 
...
sfrac %>% 
  select(rep, mapper, tid, conversion_rate, filter_type, n_fil, improvement, 
  ...
  ggplot(aes(x=conversion_rate, y=improvement, fill=filter_type)) +
    ...
    facet_grid(tx_mappability~mapper+rep) +
...
sfrac %>%
  ...
  group_by(rep, mapper, frac_filtered, tx_mappability, is_overlapping, conversion_rate, filter_type) %>% 
  ...
  geom_line() +
  geom_hline(yintercept = 0, col='black') +
  facet_grid(rep+tx_mappability~paste0(is_overlapping,', cr=',conversion_rate, ', ft=', filter_type)) +
...
best_mapper_fmat = cache({
  sfrac %>% 
    select(rep, mapper, tid, conversion_rate,filter_type, n_fil, n_overlapping, 
           true_spliced, found_spliced, found_spliced_fil, tx_mappability) %>% 
    group_by(rep, mapper, tid, n_fil, n_overlapping, true_spliced, tx_mappability, filter_type) %>% 
...
p1 = best_mapper_fmat %>% 
  group_by(rep, tx_mappability, after_fil, filter_type) %>% 
  count(best_mapper_fmat) %>% 
  ggplot(aes(x=best_mapper_fmat, y=n, fill=after_fil)) +
  geom_bar(stat='identity') +
  facet_grid(filter_type~rep) +
  ggtitle('All' )
...
p2 = best_mapper_fmat %>% 
  ...
  ggplot(aes(x=best_mapper_fmat, y=n, fill=after_fil)) +
  geom_bar(stat='identity') +
  facet_grid(filter_type~tx_mappability+rep) +
...
tab2 = perf_tx %>% 
  filter(true_isoform=='mat') %>% select(rep, tid=fid, mapper, conversion_rate, mappability, mean_map,  F1) %>% 
  group_by(trp, tid, mapper, mappability, mean_map ) %>% 
  summarise(m_F1=mean(F1, na.rm=T))

tab = tab1 %>% left_join(tab2, by=c('rep', 'tid', 'mapper'))



p1a = plot_corr(tab %>% filter(mapper=='HISAT3N', filtered=='no'), 'value', 'm_F1', 'mapper',xlog = T, 
               main_title = 'HISAT3N, not filtered', show_legend=F, alpha_=0.1) + my_scales()
p1b = plot_corr(tab %>% filter(mapper=='HISAT3N', filtered=='yes'), 'value', 'm_F1', 'mapper',xlog = T, 
               main_title = 'HISAT3N, filtered', show_legend=F, alpha_=0.1)+ my_scales()
p1c = plot_corr(tab %>% filter(mapper=='STAR', filtered=='no'), 'value', 'm_F1', 'mapper',xlog = T, 
               main_title = 'STAR, not filtered', show_legend=F, alpha_=0.1)+ my_scales()
p1d = plot_corr(tab %>% filter(mapper=='STAR', filtered=='yes'), 'value', 'm_F1', 'mapper',xlog = T,
               main_title = 'STAR, filtered', show_legend=T, alpha_=0.05)+ my_scales()
p1 = my_plot_grid(list(p1a,p1b,p1c,p1d), '')
...
my_plot_grid(list(p1,p2), 'Negative correlation between F1 (mat isoform)\nand difference to reconstructed FMAT')  
...
## GO / mappability analysis
```{r}

calc_go_results = function(go_cats=c("GO:BP")) {
  res=tibble()
  for ( map_cat in c('high', 'medium', 'low')) {
    tab=m[['tx']] %>% 
    left_join(m[['ga']],by='tid') %>% 
    mutate(ilm=ifelse(mappability==!!map_cat,1,0)) %>% 
    group_by(gene_name) %>% summarise(ilm=first(ilm), len=max(len)) %>% distinct() 
    pwf = nullp(setNames(tab$ilm, tab$gene_name), "hg38", "geneSymbol", 
            bias.data = tab$len, plot.fit=F)
    goResults <- goseq(pwf, "hg38","geneSymbol", test.cats=go_cats)
    res = res %>% bind_rows(
      goResults %>% 
        select(category, numInCat, term, numDEInCat) %>% rename(!!map_cat := numDEInCat) %>% 
        pivot_longer(!!map_cat)
    )
  }
  return(res)
}
go_results=calc_go_results()

# plot the fractions of high/med/low mappability genes for largest GO-term categories.
p1=go_results %>% 
  pivot_wider(names_from = name, values_from = value) %>% 
  arrange(desc(numInCat)) %>% 
  head(n=20) %>% 
  mutate(all=high+medium+low,
         f_high=high/numInCat,
         f_med=medium/numInCat,
         f_low=low/numInCat,
         f_all=all/numInCat
         ) %>% 
  pivot_longer(c(f_high,f_med,f_low)) %>% 
  ggplot(aes(x=term, y=value, fill=name)) +
  geom_col() + coord_flip()


plot_top10_goterms = function(map_cat='low', go_cats=c("GO:BP")) {
  tab=m[['tx']] %>% 
    left_join(m[['ga']],by='tid') %>% 
    mutate(ilm=ifelse(mappability==!!map_cat,1,0)) %>% 
    group_by(gene_name) %>% summarise(ilm=max(ilm), len=max(len)) %>% distinct() 
  pwf = nullp(setNames(tab$ilm, tab$gene_name), "hg38", "geneSymbol", 
            bias.data = tab$len, plot.fit=F)
  goResults <- goseq(pwf, "hg38","geneSymbol", test.cats=go_cats)
  p=goResults %>% 
...
p2 = plot_top10_goterms(map_cat='high') + 
  scale_colour_continuous(guide = "none") +
  ggtitle("High mappability genes") +
  ylab("")
p3 = plot_top10_goterms(map_cat='medium') + 
  scale_colour_continuous(guide = "none") +
  ggtitle("Medium mappability genes")+
  ylab("")
p4 = plot_top10_goterms(map_cat='low') + 
  scale_colour_continuous(guide = "none")  +
  ggtitle("Low mappability genes")+
  ylab("")

my_plot_grid(list(p1, p2, p3, p4), 'Fig1d', labels = T)
```
...
f2a = fcr_perf %>% 
  ...
  ggtitle("Mean difference to simulated FCR in exonic regions")
