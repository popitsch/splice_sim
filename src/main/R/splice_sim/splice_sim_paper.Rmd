...
- The target coverage is ~100X per isoform
- Show the distribution per isoform/feature/whole-tx

<div class = "blue">
<b>Coverage of simulated reads.</b>
- per transcript, feature and isoform.

</div>

```{r}
p1 = d[['tx']] %>% 
      group_by(mapper, conversion_rate, fid, len, classification ) %>% 
      summarise(count=sum(count)) %>% 
      pivot_wider(names_from=classification, values_from=count, names_sort=T) %>% 
      mutate(across(where(is.numeric), ~ifelse(is.nan(.) | is.na(.), 0, .))) %>% 
      mutate(
        true_cov=(TP+FN)*!!conf$readlen/len,
        found_cov=(TP+FP)*!!conf$readlen/len
    ) %>% 
  ggplot(aes(mapper, cov)) + geom_boxplot() +
  scale_y_sqrt() +
  geom_hline(yintercept = c(0,50,100), linetype='dotted', col='grey') + 
  ggtitle('Coverage over tx interval')


p2 = d[['fx']] %>% 
  group_by(mapper, conversion_rate, fid, true_isoform, ftype, len, classification ) %>% 
      summarise(count=sum(count)) %>% 
      pivot_wider(names_from=classification, values_from=count, names_sort=T) %>% 
      mutate(across(where(is.numeric), ~ifelse(is.nan(.) | is.na(.), 0, .))) %>% 
      mutate(
        read_count=TP+FN,
        cov=read_count*!!conf$readlen/len
    ) %>% 
  ggplot(aes(mapper, cov)) + geom_boxplot() +
  facet_grid(true_isoform~ftype) +
  scale_y_sqrt() +
  geom_hline(yintercept = c(0,50,100), linetype='dotted', col='grey') + 
  ggtitle('Coverage per feature type')

p3 = d[['tx']] %>% 
      group_by(mapper, conversion_rate, fid, true_isoform, len, classification ) %>% 
      summarise(count=sum(count)) %>% 
      pivot_wider(names_from=classification, values_from=count, names_sort=T) %>% 
      mutate(across(where(is.numeric), ~ifelse(is.nan(.) | is.na(.), 0, .))) %>% 
      mutate(
  ...
  ggplot(aes(mapper, cov)) + geom_boxplot() +
  facet_grid(.~true_isoform) +
  scale_y_sqrt() +
  geom_hline(yintercept = c(0,50,100), linetype='dotted', col='grey') + 
  ggtitle('Coverage per isoform')


my_plot_grid(list(p1,p2,p3), labels=T, 'Coverage', nrow = 1)
