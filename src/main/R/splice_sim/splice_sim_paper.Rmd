...
params:
    ...
    decay_config:
       value: "/groups/ameres/Niko/projects/Ameres/splicing/splice_sim/testruns/decay_sim_nf3/config.json"
    out_dir:
...
# decay experiment
# ===========================================================
decay_conf=fromJSON(paste(readLines(params$decay_config), collapse=""))
decay_home_dir=paste0(dirname(params$decay_config),'/')
decay_d=list()
decay_m=list()
for ( tp in decay_conf$timepoints) {
  decay_d[[paste0('tp', tp)]] = readRDS(paste0(decay_home_dir,'tp',tp,'/eva/results/data.rds'))
  decay_m[[paste0('tp', tp)]] = readRDS(paste0(decay_home_dir,'tp',tp,'/eva/results/meta.rds'))
}
decay_isoform_truth = load_table(paste0(decay_home_dir,decay_conf$isoform_config)) %>% rename(tid=transcript_id)
```
...
fcr_perf = fcr %>%
  ...
  mutate(diff_HISAT3N=HISAT3N-simulated, 
         ...
         best_mapper=ifelse(abs(diff_HISAT3N)<0.01 & abs(diff_STAR)<0.01,'BOTH',ifelse(abs(diff_HISAT3N)<abs(diff_STAR), 'HISAT3N', 'STAR'))
         )
```
...
fcr_perf %>% 
  ...
  geom_boxplot() +
  facet_zoom(ylim=c(-0.05, 0.05)) +
  ggtitle('FCR difference to simulated values per mapper')
```
...
for ( tp in decay_conf$timepoints ) {
  truth = truth %>% rbind(
    decay_isoform_truth  %>% mutate(tp=!!tp, ftc=exp(!!tp * -k)) %>% mutate(mapper='truth')
  )
}


decay_fcr = list()
for ( tp in names(decay_d)) {
  decay_fcr[[tp]] = decay_d[[tp]][['tx']] %>% 
...
decay_fcr = bind_rows(decay_fcr, .id = 'tp') %>% 
  mutate(tp=as.numeric(substr(tp,3, length(tp)))) %>% 
  left_join(decay_m[['tp0']][['tx']], by=c('fid'='tid')) # any tp will work here

# calc fcr
decay_fcr = decay_fcr %>% 
  select(tp, fid,ftype,mappability,true_fcr1, found_fcr1) %>% 
  ungroup() %>% 
  pivot_wider(names_from=mapper, values_from=c(true_fcr1, found_fcr1)) %>% 
  mutate(across(where(is.numeric), ~ifelse(is.nan(.) | is.na(.), 0, .))) %>%   # replace NA with 0; NA are resulting from mat not having introns. So if there are no FP values then there are no entries!
  rename(simulated=true_fcr1_HISAT3N, HISAT3N=found_fcr1_HISAT3N, STAR=found_fcr1_STAR) %>%
  ...
  select(tid=fid, tp, ftc, mapper) %>% 
  left_join(decay_isoform_truth, by='tid')

# add truth
decay_fcr = decay_fcr %>% 
  bind_rows(truth) %>% 
  left_join(decay_m[['tp0']][['ga']]) %>% 
   mutate(k=factor(k),
...
decay_fcr = decay_fcr %>% 
  left_join(decay_fcr %>% filter(tp==0) %>% select(tid, mapper, ftc0=ftc), by=c('tid', 'mapper')) %>% 
  mutate(ftc_norm=ftc/ftc0) %>% 
  mutate(mapper=factor(mapper, levels=c('truth', 'simulated', 'HISAT3N', 'STAR'))) %>% 
  mutate(mappability=factor(mappability, levels=c('low', 'medium', 'high')))

# calculate half-life
decay_halflifes = decay_fcr %>% 
  select(tid, mapper, mappability, decay_rate, tp, ftc_norm) %>% 
  mutate(tp = paste0('tp',tp)) %>% 
  pivot_wider(names_from = tp, values_from = ftc_norm, id_cols = c('tid', 'mapper', 'mappability', 'decay_rate') ) %>% 
  rowwise() %>% 
  mutate(hl=fit_halflife(c(0,5,10,15,20,25), c(tp0,tp5,tp10,tp15,tp20,tp25))[['hl']]) %>% 
  mutate(mappability=factor(mappability, levels=c('low', 'medium', 'high')))

# replace inf and NA with 0
decay_halflifes = decay_halflifes %>% 
  mutate_if(is.numeric, list(~na_if(., Inf))) %>% 
  replace(is.na(.), 0)

# plot halflifes
p1 = decay_fcr %>%
  ggplot(aes(x=tp, y=ftc_norm, col=mappability, group=paste0( tid, mappability,mapper))) + 
  geom_line(alpha=0.3) +
  facet_wrap(mappability~mapper) +
  geom_hline(yintercept = 0.5, col='grey', linetype='dotted') +
  facet_grid(mappability~mapper) +
  theme(legend.position="none") +
  ggtitle("Fraction of converted reads", "normalized to 1st timepoint") +
  ylim(0,1)

  
p2 = decay_halflifes %>% 
  ggplot(aes(x=mapper, y=hl, col=mappability)) + 
  geom_jitter() +
  facet_grid(mappability~mapper) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  facet_grid(mappability~mapper, scales = 'free') +
  ggtitle("Fitted halflifes")

hlcorr=decay_halflifes %>% pivot_wider(names_from = mapper, values_from = hl, id_cols = c('tid', 'mapper', 'mappability', 'decay_rate'))
p3 = plot_corr( hlcorr, 'simulated', 'STAR', 'mappability', main_title='STAR', max_x=NA, xlog=T, show_legend=F )
p4 = plot_corr( hlcorr, 'simulated', 'HISAT3N', 'mappability', main_title='HISAT3N', max_x=NA, xlog=T, show_legend=F )


my_plot_grid(list(p1,p2,p3,p4),  'Halflife reconstruction performance', labels = T, ncol = 2)
```
