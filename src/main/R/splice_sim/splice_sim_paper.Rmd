...
filter(tid %in% (decay_m[['tp0']][['tx']] %>% pull(tid))) # remove tids w/o mappings
...
my_plot_grid(list(p1,p2,p3,p4), 'bam stats', labels = T)
...
for ( tp in names(decay_d)) {
  ...
  all_data = expand_grid(fid = decay_m[['tp0']][['tx']] %>% pull(tid), 
                         mapper= c('STAR', 'HISAT3N'))
  decay_fcr[[tp]] = all_data %>% 
    left_join(decay_d[[tp]][['tx']] %>% 
...
# calc fcr
decay_fcr = decay_fcr %>% 
  select(mapper, tp, fid,ftype,mappability,true_fcr1, found_fcr1) %>% 
  ungroup() %>% 
  pivot_wider(names_from=mapper, values_from=c(true_fcr1, found_fcr1)) %>% 
  mutate(across(where(is.numeric), ~ifelse(is.nan(.) | is.na(.), 0, .))) %>%   # replace NA with 0; NA are resulting from mat not having introns. So if there are no FP values then there are no entries!
  rename(simulated=true_fcr1_HISAT3N, HISAT3N=found_fcr1_HISAT3N, STAR=found_fcr1_STAR) %>%
...
decay_fcr = decay_fcr %>% 
  group_by(mapper, tid) %>% mutate(ftc_max=max(ftc)) %>% ungroup() %>% 
  left_join(decay_fcr %>% filter(tp==0) %>% select(tid, mapper, ftc0=ftc), by=c('mapper', 'tid')) %>% 
...
decay_halflifes = decay_fcr %>% 
  select(tid, mapper, mappability, decay_rate, tp, ftc_norm_max) %>% 
  mutate(tp = paste0('tp',tp)) %>% 
  pivot_wider(names_from = tp, values_from = ftc_norm_max, id_cols = c('tid', 'mapper', 'mappability', 'decay_rate') ) %>% 
  rowwise() %>% 
...
p2 = decay_halflifes %>% 
  ...
  geom_boxplot() +
  stat_summary(fun.data = function(x){return(c(y=0,label=length(x)))}, geom = "text", position = position_dodge(width = 0.75)) +
  scale_y_sqrt() +
  ggtitle('Reconstructed halflifes')

hlcorr=decay_halflifes %>% 
  pivot_wider(names_from = mapper, values_from = hl, id_cols = c('tid', 'mapper', 'mappability', 'decay_rate'))

p3 = plot_corr( hlcorr, 'simulated', 'STAR', 'decay_rate', main_title='STAR', max_x=NA, xlog=T, show_legend=F ) +
  geom_vline(xintercept = unique(truth$hl), col='red', linetype='dotted')
p4 = plot_corr( hlcorr, 'simulated', 'HISAT3N', 'decay_rate', main_title='HISAT3N', max_x=NA, xlog=T, show_legend=F ) +
  geom_vline(xintercept = unique(truth$hl), col='red', linetype='dotted')

my_plot_grid(list(p1,p2,p3,p4),  'Halflife reconstruction performance', labels = T, ncol = 2)
ggsave(paste0(params$out_dir, 'special_decay.pdf'), width=12, height=10)
...
out_tids = hlcorr %>% filter(cat=='Outlier in both') %>% pull(tid)
p4 = decay_fcr %>%
...
my_plot_grid(list(p1,p2,p3,p4), 'Halflife outliers', labels = T, ncol = 2)
ggsave(paste0(params$out_dir, 'special_decay_outliers.pdf'), width=12, height=10)
```
