...
### BAM stats
@see splice_sim_special_hisat3n_analysis.Rmd
...
for ( r in c('rep1', 'rep2', 'rep3')) {
  ...
  p3 = dr[['tx']] %>% filter(rep==!!r) %>% 
    group_by(mapper, conversion_rate) %>% 
    summarise(TP=sum(all_TP), FN=sum(all_FN), FP_raw=sum(all_FP_raw)) %>% 
    pivot_longer(c(TP, FN, FP_raw)) %>% 
    ggplot(aes(conversion_rate, value, fill=name)) + geom_col() + 
    facet_wrap(mapper~.) + ggtitle('read counts per mapper/condition') 
...
isoform_mix = load_table('/groups/ameres/Niko/projects/Ameres/splicing/data/200513_R9575_pulse_reseq/3_map/STAR/mm10/ns_r1_ip_180/ns_r1_ip_180.gencode.vM21.annotation.nochr.sorted.MAXISOFORM.gff3.data.tsv') %>% as_tibble()
...
ggsave(paste0(params$out_dir, 'qc_isoform_selection.pdf'), width=12, height=10)
```
...
dr_sample = dr[['tx']] %>% 
  ...
  group_by(fid, rep, mappability)  %>% 
  summarise(all_true=mean(all_true), all_found=mean(all_found),
            tc1_true=mean(tc1_true), tc1_found=mean(tc1_found)) %>% 
  pivot_wider(names_from = rep, values_from = c(all_true, all_found, tc1_true, tc1_found)) %>% 
  ungroup()

p1 = dr_sample %>% plot_corr('all_true_rep1', 'all_true_rep2', col_ = 'mappability', xlog=TRUE, use_scattermore=TRUE, show_legend=FALSE, main_title = " ") + 
  xlab("all simulated rep1") + ylab("all simulated rep2")

p2 = dr_sample %>% plot_corr('all_true_rep1', 'all_found_rep1', col_ = 'mappability', xlog=TRUE, use_scattermore=TRUE, show_legend=FALSE, main_title = " ") + 
  xlab("all simulated rep1") + ylab("all mapped rep1")

p3 = dr_sample %>% plot_corr('all_found_rep1', 'all_found_rep2', col_ = 'mappability', xlog=TRUE, use_scattermore=TRUE, show_legend=FALSE, main_title = " ") + 
  xlab("all mapped rep1") + ylab("all mapped rep2")

p4 = dr_sample %>% plot_corr('tc1_found_rep1', 'tc1_found_rep2', col_ = 'mappability', xlog=TRUE, use_scattermore=TRUE, show_legend=TRUE, main_title = " ") + 
  xlab("tc1 mapped rep1") + ylab("tc2 mapped rep2") + 
...
calculation that does not take partial read overlap into account. 
</div>
...
calc_coverage = function(tab, per_rep=F) {
  grp=c('ftype', 'true_isoform', 'conversion_rate', 'fid')
  if ( per_rep ) {
    grp = c(grp, 'rep')
  }
  tab %>% 
    lazy_dt() %>% 
    select(mapper, fid, ftype, true_isoform, conversion_rate, len, all_true, all_found, tc1_true, tc1_found) %>% 
    ...
    select(mapper, fid, ftype, true_isoform, conversion_rate,
...
p3 = tab %>% 
  ...
  scale_fill_manual(values = my_colors, limits = force, guide = guide_legend(override.aes = list(color = "white", fill= "white"))) # invisible legend
...
dp[['tx']] %>% 
  group_by( mapper, true_isoform, ftype, mappability, convertibility, conversion_rate) %>% 
  calc_iqr(all_F1) %>% 
  ggplot(aes(as.numeric(as.character(conversion_rate))*100, col.median, col=mapper, group=paste0(mapper,true_isoform), linetype=true_isoform)) +
  geom_line() +
  #geom_ribbon(aes(ymin=col.lower, ymax=col.upper), linetype=2, alpha=0.2) +
...
tab %>% select(chromosome,start,end,best_mapper=best_mapper_conv,score_conv,strand,start1,end1) %>%
  mutate(rgb=ifelse(best_mapper=='HISAT3N','238,118,0',ifelse(best_mapper=='STAR','0,139,0', '139,105,20'))) %>% 
      write_bed(paste0(params$out_dir,'/best_mapper_map.include_converted.bed'),
                paste0("best_mapper_map.include_converted"))

tab %>% select(chromosome,start,end,best_mapper=best_mapper_no_conv,score_unconv,strand,start1,end1) %>%
  mutate(rgb=ifelse(best_mapper=='HISAT3N','238,118,0',ifelse(best_mapper=='STAR','0,139,0', '139,105,20'))) %>% 
      write_bed(paste0(params$out_dir,'/best_mapper_map.unconverted_only.bed'),
                paste0("best_mapper_map.unconverted_only"))

tab %>% select(chromosome,start,end,best_mapper=best_mapper_tc1,score_tc1,strand,start1,end1) %>%
  mutate(rgb=ifelse(best_mapper=='HISAT3N','238,118,0',ifelse(best_mapper=='STAR','0,139,0', '139,105,20'))) %>% 
      write_bed(paste0(params$out_dir,'/best_mapper_map.tc1.bed'),
                paste0("best_mapper_map.tc1"))

tab %>% select(chromosome,start,end,best_mapper=best_mapper_tc2,score_tc2,strand,start1,end1) %>%
...
tab = sfrac %>% 
  ...
  ungroup() %>% 
  mutate(simulated=ifelse(is.na(HISAT3N_true_spliced),STAR_true_spliced,HISAT3N_true_spliced)) %>% 
  rename(HISAT3N=HISAT3N_found_spliced, 
...
tab %>% ggplot(aes(name, value, fill=mapper, col=mapper, alpha=0.5)) +
  geom_boxplot() +
  #geom_violin(draw_quantiles = c( 0.5)) +
  #stat_summary(fun.data = "mean_cl_boot", geom = "pointrange") +
  facet_grid(.~tx_mappability) +
  #scale_y_log10() +
  geom_hline(yintercept = 1/3, col='black', linetype='dashed') +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  xlab("") + ylab("FMAT") +
  my_scales() + guides(alpha = "none") +
  stat_summary(fun.data = function(x){return(c(y=-0.03,label=length(x)))}, 
...
my_plot_grid(list(p1,p2), 'Read classification per mapper', labels = T, rel_widths = c(1,1.1))
...
home_dir=list(
  "rep1" = "/groups/ameres/Niko/projects/Ameres/splicing/splice_sim/testruns/big4_slamseq_nf_kss/",
  "rep2" = "/groups/ameres/Niko/projects/Ameres/splicing/splice_sim/testruns/big4_slamseq_nf_rep2_kss/",
  "rep3" = "/groups/ameres/Niko/projects/Ameres/splicing/splice_sim/testruns/big4_slamseq_nf_rep3_kss/"
)
fcr_cvsu=cache({
  tab=tibble()
